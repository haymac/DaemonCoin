...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeDaemonAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDaemonAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeDaemonAmount = strAnonymizeDaemonAmount.remove(strAnonymizeDaemonAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDaemonAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeDaemonAmount*COIN) nMaxToAnonymize = nAnonymizeDaemonAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeDaemonAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeDaemonAmount));
        strAnonymizeDaemonAmount = strAnonymizeDaemonAmount.remove(strAnonymizeDaemonAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDaemonAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
