...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeDaemonAmount"))
        settings.setValue("nAnonymizeDaemonAmount", 1000);
    nAnonymizeDaemonAmount = settings.value("nAnonymizeDaemonAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeDaemonAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeDaemonAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeDaemonAmount:
            return QVariant(nAnonymizeDaemonAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeDaemonAmount:
            nAnonymizeDaemonAmount = value.toInt();
            settings.setValue("nAnonymizeDaemonAmount", nAnonymizeDaemonAmount);
            emit AnonymizeDaemonAmountChanged(nAnonymizeDaemonAmount);
            break;
        default:
            break;
        }
