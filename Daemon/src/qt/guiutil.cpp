...
{
    // Convert Daemon:// to Daemon:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Daemon://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Daemon:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Daemon.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Daemon.lnk
...
{
    return GetAutostartDir() / "Daemon.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Daemon.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Daemon\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Daemon app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Daemon app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Daemon-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Daemon-qt [" + tr("command-line options") + "]                     " + "\n";
