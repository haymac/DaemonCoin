...
int nAnonymizeDaemonAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Daemon
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Daemon
    // Mac: ~/Library/Application Support/Daemon
    // Unix: ~/.Daemon
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Daemon";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Daemon.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Daemond.pid"));
