...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Daemon.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Daemond.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeDaemonamount=<n> " + _("Keep N Daemon anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Daemon is shutting down."));
    ...
    if (fDaemon)
        fprintf(stdout, "Daemon server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Daemon") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Daemon to complete") << "\n";
    ...
    nAnonymizeDaemonAmount = GetArg("-anonymizeDaemonamount", 0);
    if(nAnonymizeDaemonAmount > 999999) nAnonymizeDaemonAmount = 999999;
    if(nAnonymizeDaemonAmount < 2) nAnonymizeDaemonAmount = 2;
    ...
    LogPrintf("Anonymize Daemon Amount %d\n", nAnonymizeDaemonAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1DMN+1000 == (.1DMN+100)*10
       10DMN+10000 == (1DMN+1000)*10
    */
